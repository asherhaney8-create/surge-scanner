...
def render_whale_tracker():
    st.markdown("### 🐋 Whale tracker · Hyperliquid")
    st.caption("Crypto is transparent — every wallet's trades are public on-chain. This tracks large "
               "Hyperliquid perpetuals traders: their live positions, recent win rate, and realized PnL. "
               "Perps only · not affiliated with Hyperliquid · crypto is volatile and this is not financial advice.")
    ...
    if not follows:
        st.info("You're not following any whales yet. Paste a wallet above, or tap **Follow** on the "
                "leaderboard below. Your list is saved in this page's link — bookmark or share it to keep it.")
    else:
...
def render_gamble():
    st.markdown("### 🎲 Gamble · Polymarket macro bets")
    st.caption("Prediction markets are transparent — big bets and the wallets making them are public. This "
               "surfaces whales putting real size on major macro & geopolitical questions. Polymarket is "
               "real-money betting, availability varies by jurisdiction, and this is not financial or betting advice.")
...
section = st.radio("Section", ["📈 Stock scanner", "🐋 Whale tracker", "🎲 Gamble"],

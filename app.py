...
def _render_topic(cat, bets, markets):
    ...
    st.markdown(f"##### 🚨 Big {cat} bets — live")
    if not cbets:
        ...
        if recent:
            ...
            st.warning(f"🐋 Biggest in the last hour: **${tb['usd']:,.0f}** on "
                       f"**{tb['side']} {tb['outcome']}** — {tb['title']}", icon="🚨")
        for b in cbets[:20]:
...
def render_gamble():
    ...
    icons = {"Politics": "🏛️", "Geopolitics": "🌍", "Fed & economy": "🏦",
             "Crypto": "🪙", "Tech & AI": "🤖"}
    tabs = st.tabs([f"{icons[c]} {c} ({counts[c]})" for c in CAT_ORDER])

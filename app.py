...
def render_whale_tracker():
    section_header("Whale tracker · Hyperliquid", "Follow the whales",
                   "Crypto is transparent — every wallet's trades are public on-chain. Track large "
                   "Hyperliquid perpetuals traders: live positions, recent win rate, and realized PnL.")
...
def render_sp_spikes():
    ...
    with right:
        ...
        st.dataframe(disp, hide_index=True, use_container_width=True, height=330, column_config={
...
with c1:
    ...
    <div class="beacon-row"><div class="beacon"></div>
      ...
      <div class="tagline">what's in play, right now</div></div></div>
    """, unsafe_allow_html=True)
with c2:
    now = et_now().strftime("%I:%M %p ET")
    st.markdown(f"<div style='text-align:right;color:#6b7690;font-size:12px;margin-top:10px'>"
                f"updated {now}<br>auto-refresh · 2 min</div>", unsafe_allow_html=True)

st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)

# ---- top-level section switch ----
section = st.radio("Section", ["📈 Stock scanner", "🔥 S&P Spikes", "🐋 Whale tracker", "🎲 Gamble"],

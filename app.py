...
:root { --accent:#22c58a; --accent-soft:#12271e; --accent-ink:#5ef2b3;
        --ink:#e9edf5; --ink2:#a6b0c3; --ink3:#6b7690;
        --surface:#131822; --surface2:#1a2130; --line:#232b3a; --line2:#37415a;
        --up:#22c58a; --down:#f0685a; }
...
.logo{width:40px;height:40px;flex:none;filter:drop-shadow(0 2px 7px rgba(34,197,138,.28));}
.wordmark{font-family:"Archivo",sans-serif;font-weight:800;font-size:26px;letter-spacing:-.6px;line-height:1;}
.wordmark span{color:var(--accent);}
.tagline{font-size:12px;color:var(--ink3);letter-spacing:.3px;margin-top:3px;text-transform:uppercase;font-weight:600;}
...
.chip .k{width:19px;height:19px;border-radius:50%;display:grid;place-items:center;font-size:11px;color:#08120d;font-weight:700;background:var(--accent);}
...
div[role="radiogroup"] > label:has(input:checked) p{ color:var(--accent-ink); font-weight:600; }
...
button[data-baseweb="tab"][aria-selected="true"]{ color:var(--accent-ink); }
...
div.stButton > button:hover{ border-color:var(--accent); color:var(--accent-ink); }

/* dataframe frame */
[data-testid="stDataFrame"]{ border:1px solid var(--line); border-radius:12px; }

.tag{font-size:9.5px;font-weight:700;letter-spacing:.4px;padding:2px 6px;border-radius:5px;}
.tag.low{color:var(--accent-ink);background:var(--accent-soft);} .tag.mid{color:var(--ink2);background:#212836;}
...
[data-testid="stSidebar"] div[role="radiogroup"] > label:has(input:checked) p{ color:var(--accent-ink); }
...
def render_whale_tracker():
    ...
    if not follows:
        ...
        for a in follows:
            ...
            h1.markdown(f"**`{_short(a)}`**  " + ("· active now" if active else ""))
    ...
    if not lb:
        ...
        return
    ctl = st.columns([2.4, 1.6])
    rank_by = ctl[0].radio("Rank by", ["Today's volume", "All-time PnL", "ROI"],
                           horizontal=True, key="wlrank",
                           help="'Today's volume' surfaces the whales actually moving size right now.")
    winners_only = ctl[1].checkbox("Win rate ≥ 50% only", value=True,
                                   help="Only show whales whose recent trades are profitable more than half the time.")
    key = {"All-time PnL": "pnlAll", "Today's volume": "vlm", "ROI": "roiAll"}[rank_by]
    pool = sorted(lb, key=lambda x: x.get(key, 0), reverse=True)[:20]

    with st.spinner("Checking win rates…"):
        rows = []
        for r in pool:
            a = r["address"].lower()
            wr = hl_winrate(a)
            r["_wr"] = wr["winrate"] if wr else None
            r["_wrtxt"] = f"{wr['winrate']:.0f}%  ({wr['wins']}/{wr['total']})" if wr else "—"
            if winners_only and (r["_wr"] is None or r["_wr"] < 50):
                continue
            rows.append(r)
            if len(rows) >= 15:
                break
    ...
    if not rows:
        st.info("None of the current top traders have a recent win rate ≥ 50%. Untick the filter to see them all "
                "(remember: big all-time PnL doesn't always mean a high hit-rate).")
    for r in rows:
        a = r["address"].lower()
        c = st.columns([2.6, 2.2, 2.2, 1.9, 1.4])
        c[0].markdown(f"`{_short(a)}`")
        wr_color = "#2ecb8f" if (r["_wr"] is not None and r["_wr"] >= 50) else "#a6b0c3"
        c[1].markdown(f"WR <b style='color:{wr_color}'>{r['_wrtxt']}</b>", unsafe_allow_html=True)
        c[2].markdown(f"PnL **${r['pnlAll']:,.0f}**")
        c[3].markdown(f"Vol/day ${r['vlm']:,.0f}")
        if a in follows:
            c[4].markdown("✓ following")
        elif c[4].button("Follow", key="fol_" + a):
            follows.append(a); set_follows(follows); st.rerun()
    st.caption("Win rate is computed from each wallet's recent fills (not its entire lifetime), so treat it as a "
               "guide. Leaderboard is Hyperliquid's public data and can lag or change.")


# ============================================================================
#  GAMBLE (Polymarket) — big bets, organised by topic
# ============================================================================
PM_GAMMA = "https://gamma-api.polymarket.com/markets"
PM_TRADES = "https://data-api.polymarket.com/trades"

# Each topic → its keywords. Classification runs in this order (a title lands in the
# first topic it matches), so put the more specific topics before the broad ones.
CATEGORY_KWS = {
    "Fed & economy": ["fed", "fomc", "powell", "interest rate", "rate cut", "rate hike",
                      "rate decision", "basis point", "bps", "inflation", "cpi", "pce", "gdp",
                      "recession", "unemployment", "jobs report", "jobless", "debt ceiling",
                      "government shutdown", "tariff", "s&p 500", "stock market"],
    "Geopolitics": ["iran", "israel", "russia", "ukraine", "china", "taiwan", "war", "invade",
                    "invasion", "nuclear", "missile", "ceasefire", "nato", "gaza", "north korea",
                    "sanction", "blockade", "venezuela", "hamas", "hezbollah", "troops", "putin"],
    "Crypto": ["bitcoin", "btc", "ethereum", "eth", "solana", "crypto", "token", "memecoin",
               "stablecoin", "altcoin", "dogecoin", "xrp", "binance", "coinbase", "halving"],
    "Tech & AI": ["openai", "chatgpt", "gpt", "artificial intelligence", "ai", "nvidia", "tesla",
                  "spacex", "starship", "tiktok", "semiconductor", "apple", "google", "microsoft"],
    "Politics": ["election", "president", "presidential", "congress", "senate", "governor",
                 "nomination", "nominee", "primary", "vote", "poll", "trump", "biden", "harris",
                 "democrat", "republican", "supreme court", "impeach", "cabinet", "mayor",
                 "parliament", "prime minister", "nobel"],
}
# obvious noise we never want in a topic feed
NOISE = ["up or down", "updown", "counter-strike", "cs2", " vs ", "vs.", "bo3", "bo5",
         "nba", "nfl", "mlb", "nhl", "ufc", "premier league", "la liga", "champions league"]
# order the tabs are shown in (most-followed first)
CAT_ORDER = ["Politics", "Geopolitics", "Fed & economy", "Crypto", "Tech & AI"]
CLASSIFY_ORDER = ["Fed & economy", "Geopolitics", "Crypto", "Tech & AI", "Politics"]

def _kw_hit(kw, text):
    if " " in kw:                      # multi-word phrase → plain substring
        return kw in text
    return re.search(r"\b" + re.escape(kw) + r"\b", text) is not None   # single word → whole word

def classify(title):
    """Return the topic for a market/bet, or None (noise / off-topic → dropped)."""
    t = (title or "").lower()
    if not t or any(n in t for n in NOISE):
        return None
    for cat in CLASSIFY_ORDER:
        if any(_kw_hit(kw, t) for kw in CATEGORY_KWS[cat]):
            return cat
    return None
...
@st.cache_data(ttl=300, show_spinner=False)
def pm_markets():
    """Top prediction markets by volume, tagged with a topic."""
    try:
        r = requests.get(PM_GAMMA, params={"closed": "false", "active": "true",
                         "limit": 400, "order": "volumeNum", "ascending": "false"},
...
def pm_big_bets(min_usd):
    """Recent large trades (>= min_usd), tagged with a topic."""
    try:
...
def _render_topic(cat, bets, markets):
    """One topic tab: its big bets feed + its markets to watch."""
    cbets = [b for b in bets if b["cat"] == cat]
    cmkts = [m for m in markets if m["cat"] == cat]

    st.markdown(f"##### Biggest {cat} bets")
    if not cbets:
        st.info(f"No {cat} bets above that size in the recent feed. Lower the minimum, or check back — "
                "big bets come in bursts.")
    else:
        recent = [b for b in cbets if b["mins"] <= 60]
        if recent:
            tb = max(recent, key=lambda x: x["usd"])
            st.markdown(
                ...
                f"Biggest in the last hour — <b>${tb['usd']:,.0f}</b> on "
                f"<b>{tb['side']} {tb['outcome']}</b> · {tb['title']}</div>",
                unsafe_allow_html=True)
        for b in cbets[:20]:
    ...
    st.markdown(f"##### {cat} markets to watch")
    if not cmkts:
        st.caption("No active markets loaded for this topic right now.")
    else:
        df = pd.DataFrame([{
...
def render_gamble():
    section_header("Gamble · Polymarket", "Big money, by topic",
                   "Prediction markets are transparent — the big bets and the wallets behind them are public. "
                   "Pick a topic below to see the biggest bets and the markets to watch in it.")
    st.caption("Real-money betting · availability varies by jurisdiction · not financial or betting advice.")

    min_usd = st.selectbox("Minimum bet size", [1000, 5000, 10000, 25000, 50000, 100000],
                           index=2, format_func=lambda v: f"Show bets ≥ ${v:,}")

    bets = pm_big_bets(min_usd)
    markets = pm_markets()

    counts = {c: sum(1 for b in bets if b["cat"] == c) for c in CAT_ORDER}
    tabs = st.tabs([f"{c}  ({counts[c]})" for c in CAT_ORDER])
    for tab, cat in zip(tabs, CAT_ORDER):
        with tab:
            _render_topic(cat, bets, markets)

    st.caption("Odds are the market-implied probability. Topics are auto-sorted from the market title. "
               "Data from Polymarket's public API; it can lag or change.")
...
SURGE_LOGO = (
    ...
    "xmlns='http://www.w3.org/2000/svg'><rect x='0' y='0' width='40' height='40' rx='11' fill='#12271e'/>"
    "<g stroke='#22c58a' stroke-width='2.3' stroke-linecap='round'>"
    "<line x1='10' y1='25' x2='10' y2='34'/><line x1='20' y1='13' x2='20' y2='31'/>"
    "<line x1='30' y1='6' x2='30' y2='23'/></g>"
    "<g fill='#22c58a'><rect x='7' y='27' width='6' height='6' rx='1'/>"
    "<rect x='17' y='18' width='6' height='10' rx='1'/><rect x='27' y='10' width='6' height='10' rx='1'/></g>"
    "</svg></span>")

with st.sidebar:
    st.markdown(
        f"<div class='beacon-row'>{SURGE_LOGO}"
    ...
    section = st.radio("Sections",
                       ...
                       label_visibility="collapsed")
    st.markdown("<hr style='margin:16px 0 12px;border-color:#232b3a'>", unsafe_allow_html=True)
...
if section == "S&P Spikes":
    render_sp_spikes()
    st.stop()
if section == "Whale tracker":
...
st.markdown(
    "<div class='chips'>"
    f"<span class='chip'><span class='k'>1</span> Up today "
    f"<span class='v'>{chg_min:.0f}–{chg_max:.0f}%</span></span>"
    f"<span class='chip'><span class='k'>2</span> Heavy volume "
    f"<span class='v'>RVOL ≥ {rvol_min:.1f}×</span></span>"
    f"<span class='chip'><span class='k'>3</span> "
...
volume and news. A "· live" tag marks whichever session is happening right now; the others fill in during their hours.
...
def dot(s): return "  · live" if s == sess else ""
...
st.caption("Hover any column header for what it means. The \"· live\" tag marks the session that's live right now.")
...
with tab_ov:
    ...
    if news:
        ...
        st.markdown(
            f"<div style='background:#131822;border-left:3px solid var(--accent);border-radius:8px;"
    ...
    if not d["target"] or not d["n_analysts"]:
        ...
        if lo and hi and hi > lo:
            ...
            <div style='display:flex;justify-content:space-between;font-size:11px;color:#6b7690;
                 ...
                 <span><b style='color:#e9edf5'>● now</b> &nbsp; <b style='color:#22c58a'>● target</b></span><span>high ${hi:.2f}</span></div>
            """, unsafe_allow_html=True)
